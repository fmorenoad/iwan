                        ...
                        <div class="card-header">
                            <div class="row align-items-center">
                                ...
                                <div class="col-md-2 ml-auto">
                                    <a href="{{ route('admin.roles.create') }}" class="btn btn-primary btn-outline btn-wd">{{ __('New Role')}}</a>
                                </div>
                            </div>
                        </div>
                            ...
                            <table id="datatables" class="table table-striped table-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
                                ...
                                <tbody>
                                    @foreach ($roles as $role)
                                        <tr>
                                            <td>{{ $role->name }}</td>
                                            <td>{{ $role->guardname }}</td>
                                            <td class="text-center">
                                                <a href="{{ route('admin.roles.edit', ['role' => $role]) }}" class="btn btn-link btn-warning edit d-inline-block"><i class="fa fa-edit"></i></a>
                                                <form class="d-inline-block" action="{{ route('admin.roles.destroy', ['role' => $role]) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
